...
- **Every package's NuGet README was rewritten for this release.** Each one now opens with what the package
  does and a snippet you can paste and run, then the API grouped by task, the options that matter with their
  defaults, and an explicit "what this does not do" section naming each package's real edges. Every C# snippet
  was extracted and compiled against these sources, behaviour claims were re-checked against the code on this
  branch, and version numbers were removed from prose in favour of the rule behind them (a TFM, an EF Core
  major, a MassTransit range, the SDK floor the generators need). The root README's package table now lists
