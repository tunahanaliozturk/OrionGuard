...
- **Regex rules now always run with a match timeout.** `[Regex]` (`RegexAttribute`), the compatibility layer's
  ...
  `GeneratedRegexPatterns.Email()`, both bounded at 1 second, and throw `RegexMatchTimeoutException` instead of
  hanging. Validators generated by `OrionGuard.Generators` pick this up on the next build.
