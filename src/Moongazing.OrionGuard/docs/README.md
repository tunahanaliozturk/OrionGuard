...
- **`AgainstPathTraversal` normalizes with NFKC, which the browser-wasm runtime does not implement.** `string.Normalize` throws `PlatformNotSupportedException` there, so that guard — and `AgainstUnsafeFileName` and `AgainstInjection`, which share the normalization — cannot run in a Blazor WebAssembly client. Run them on the server, which is the side that has to be convinced anyway.
